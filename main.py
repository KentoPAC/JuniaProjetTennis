...
# L'ID de la balle de tennis dans COCO est généralement 32 ou 41 selon les versions
...
while cap.isOpened():
    ...
    for result in results:
        # Récupérer les résultats de la détection des objets (boîtes de détection)
        boxes = result.boxes  # Accéder aux boîtes de détection

        # Vérifier chaque détection
        for box in boxes:
            # box est un objet Box avec les coordonnées, la confiance et la classe
            x1, y1, x2, y2 = box.xywh[0]  # Coordonnées de la boîte de détection
